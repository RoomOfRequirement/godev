...
func TestNewSkipList(t *testing.T) {
	var _ container.Container = (*SkipList)(nil)

	sl := NewSkipList(-10, 0, container.IntComparator)
...
func TestSkipList_Set(t *testing.T) {
	sl := NewSkipList(8, 4, container.IntComparator)
...
func BenchmarkSkipList_Set(b *testing.B) {
	...
	sl := NewSkipList(maxLevel, 4, container.IntComparator)
	for i := 0; i < b.N; i++ {
		sl.Set(i, data[i])
	}
}
